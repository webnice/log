...
type StringFormatterStruct struct {
	parts []part
}
...
//
// Experimental:
//     %{package}   Full package path, eg. github.com/go-logging
//     %{shortpkg}  Base package path, eg. go-logging
//     %{function}  Full function name, eg. littleEndian.PutUint32
//     %{shortfunc} Base function name, eg. PutUint32
func NewStringFormatter(format string) (*StringFormatterStruct, error) {
	var fmter = &StringFormatterStruct{}
...
// on error.
func MustStringFormatter(format string) *StringFormatterStruct {
...
func (f *StringFormatterStruct) add(verb fmtVerb, layout string) {
	f.parts = append(f.parts, part{verb, layout})
}

func (f *StringFormatterStruct) Format(calldepth int, r *Record, output io.Writer) error {
