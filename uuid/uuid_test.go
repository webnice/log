...
func TestUUIDFromBytes(t *testing.T) {
	uuid, _ := ParseUUID("486f3a88-775b-11e3-ae07-d231feb1dc81")
	uuid2, err := UUIDFromBytes(uuid.Bytes())
	if err != nil {
		t.Errorf("Error UUIDFromBytes(): %v", err)
	}
	if strings.EqualFold(uuid.String(), uuid2.String()) == false {
		t.Errorf("Error UUIDFromBytes(). Reqult UUID is not equal original UUID")
	}
}
...
func TestMarshalJSON(t *testing.T) {
	...
	if strings.EqualFold(string(bt), us) == false {
		t.Errorf("Error in MarshalJSON(): %v", err)
	}
}
